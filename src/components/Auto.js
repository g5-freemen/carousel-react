...
export default function Auto(props) {
  let { autoInterval, spaceBetweenSlides, reverse = 1, setSlideNum } = props;
  const autoCheckbox = useRef(null);

  function AutoScroll() {
    const btns = document.querySelectorAll(
      ".slider-dots, .prev-btn, .next-btn"
    );
    ...
    if (
      (carousel.scrollWidth - width <= carousel.scrollLeft + width &&
        reverse > 0) ||
