...
export default function Carousel({ children }) {
  ...
  function setCarousel() {
    ...
    const slideWidth =
      (width - spaceBetweenSlides * (slidesToShow - 1)) / slidesToShow + "px";
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("resize", setCarousel);
  }, []);
  ...
  return (
    <div className="wrapper">
      ...
      {slidesToShow !== length && length && showAuto && (
        <Auto {...{ autoInterval, spaceBetweenSlides, setSlideNum }} />
      )}
      {slidesToShow !== length && length && showDots && (
        <Dots {...{ length, slideNum, setSlideNum }} />
      )}
